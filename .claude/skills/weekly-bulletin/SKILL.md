...
| 주보 항목 | 스펙 필드 |
|---|---|
| *찬 송 (첫 번째) | `hymn1` = { number, title } — `number`는 **새찬송가 번호** |
...
**찬송은 악보를 쓰지 않는다.** `number`(새찬송가 번호)만 넘기면 찬송가 DB에서 가사 슬라이드가
만들어지고 절·후렴 배치도 자동이다. 주보의 찬송이 새찬송가가 아니면(복음성가 등) `number`를 빼고
`title`만 넣는다 — 순서 슬라이드 제목은 "찬송"이 되고, 가사는 만들지 못했다고 TODO로 보고된다.

광고는 `page-2.png`의 "교/회/소/식" 번호 항목을 그 순서대로 `announcements`에 담는다.
`{ text }` / `{ image }` / `{ text, image }` 모두 가능하다(이미지만 있는 광고도 Base에 자리가 있다).
그 칸의 고정 안내(헌금 계좌, 바른 예배 캠페인)와 별도 구획인 "교/우/동/정"은 Base에 자리가 없으므로
넣지 말고, 필요해 보이면 사용자에게 물어본다.

## 3. 광고 이미지 크롭 (필요할 때만)

광고 항목이 표·그림으로 되어 있으면(예: 여름행사 안내, 특별집회 일정표) PDF에서 **400 DPI로 잘라** 쓴다.
...
찬송 **가사**·교독문·성경본문 새로 생성 → 재정렬.

블록(찬송 가사·교독문·성경본문·광고)은 요소 종류가 아니라 **위치**로 잡는다 — 순서 슬라이드
(큰 글씨 가운데정렬 제목) 바로 뒤에 이어지는 슬라이드들이 그 블록이다. 그래서 Base 구성이
바뀌어도(악보 이미지 → 가사 텍스트처럼) 안 깨진다.

`--dry-run`이 "Base에서 ... 못 찾음"으로 실패하면 Base 구조가 바뀐 것이다. 임의로 고치지 말고
`data/base-backups/`의 최신 백업과 비교해 **사용자에게 물어본다** — 사용자가 직접 Base를
손보는 일이 잦다(장수는 고정이 아니다).

## 5. 보고

만든 예배 id·장수와 함께, 스크립트가 출력한 **"손대지 않음"** 항목을 그대로 전달한다.
그 목록은 Base에 실제로 남아 있는 것만 나오므로 **추측을 보태지 말 것.** 흔히 나오는 것:

- **찬양대 가사** — 곡 제목만 바뀌고 크로마키 가사는 지난주 것 그대로. 매주 수동 입력 필요.
- **새가족 명단** — 주보에 없음.
- **번호 없는 찬송** — 새찬송가가 아니면 가사를 만들지 못한다.
