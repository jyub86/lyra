# Sunday Worship PPT — 전체 설계 문서 (v2)

> 작성일: 2026-06-24
> 스택: Bun + 순수 HTML/JS + SQLite
> 실행 환경: M1 Max MacBook Pro (로컬 전용)
> v2 핵심: Service>Scene>Slide 계층 / 템플릿 / 레이어(영상) / **MCP·CLI 우선 Tool-First 아키텍처**
...
```
Service (예배 덱)         "2025-06-29 1부 예배"
  └─ Scene (씬)           "예배로 부름" / "찬양" / "말씀" / "광고"
       └─ Slide (슬라이드)  개별 화면 (분할 페이지 포함)

Template (템플릿)         슬라이드/씬을 생성하는 "생성기" (계층 밖, 도구)
Theme (테마)             시각 스타일 (계층 밖, Service에 적용)
Scene Library (씬 보관함)  재사용 가능한 씬 저장소 (공유용)
```

| 개념 | 정의 | 저장 | 공유 |
|------|------|------|------|
| Service | 한 번의 예배 전체 덱 | DB | 덱 복사 |
| Scene | 슬라이드 묶음 (순서 단위) | DB + Library | JSON export/import |
| Slide | 개별 화면 | DB | (씬 단위로) |
...
CREATE TABLE scenes (
  ...
  position    INTEGER NOT NULL,        -- 서비스 내 씬 순서
  name        TEXT NOT NULL,           -- "예배로 부름", "찬양", "광고"
  FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE
);
CREATE INDEX idx_scenes_service ON scenes(service_id, position);

CREATE TABLE slides (
  id            TEXT PRIMARY KEY,      -- ulid
  scene_id      TEXT NOT NULL,         -- ★ 이제 scene에 속함
  position      INTEGER NOT NULL,      -- 씬 내 슬라이드 순서
...
CREATE INDEX idx_slides_scene ON slides(scene_id, position);

-- ===== Scene Library (재사용/공유) =====
CREATE TABLE scene_library (
  id          TEXT PRIMARY KEY,        -- ulid
  name        TEXT NOT NULL,
  theme_hint  TEXT,                    -- 권장 테마 (선택)
...
```
list_services()                                  → 덱 목록
get_service(service_id)                           → 덱 + 씬 + 슬라이드 트리
get_scene(scene_id)                               → 씬 상세
...
update_service(service_id, fields)
duplicate_service(service_id)                              → new_service_id
delete_service(service_id)
set_service_theme(service_id, theme_id)

# Scene
add_scene(service_id, name, position?)                    → scene_id
update_scene(scene_id, fields)
reorder_scenes(service_id, ordered_scene_ids)
remove_scene(scene_id)
export_scene(scene_id)                                     → scene JSON
import_scene(service_id, scene_json, position?)           → scene_id
save_scene_to_library(scene_id, name)                     → library_id
add_scene_from_library(service_id, library_id, position?) → scene_id

# Slide
add_slide(scene_id, template_type, data, position?, background?, overlays?, transition?) → slide_id
update_slide(slide_id, fields)
remove_slide(slide_id)
reorder_slides(scene_id, ordered_slide_ids)
set_slide_background(slide_id, background)
set_slide_overlays(slide_id, overlays)

# 콘텐츠 생성 (고가치 — LLM이 주로 사용)
add_bible_slides(scene_id, book, chapter, verse_start, verse_end, layout?)   → slide_ids
add_hymn_slides(scene_id, number, verse_nos?, lines_per_slide?)              → slide_ids
add_reading_slides(scene_id, number)                                        → slide_ids
add_praise_slides(scene_id, title, sections, lines_per_slide?)              → slide_ids
   # ★ sections = [{ label, lines:[...] }]  — 구조화된 입력만 받음.
   #   지저분한 가사 텍스트 해석은 외부 LLM이 담당 → 내부 파서 없음.
add_announcement_slide(scene_id, items)                                     → slide_id
...
| 계층 | Service > Scene > Slide | 씬 단위 저장/공유/재사용 |
...
| 씬 공유 | worship-scene/v1 JSON + asset 참조 | 가볍고 LLM 친화적 |
