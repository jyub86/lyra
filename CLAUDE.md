...
>   글자 내용은 아예 못 바꿨고, "한 장 꾸미고 → 퍼뜨리기" 2단계라 즉석 수정에 안 맞았다.
> - **짝짓기 규칙은 `core/lib/element-match.js` 하나뿐**(`matchKeys`/`keyLabel`). 서버 도구와
>   편집기가 **같은 파일**을 쓴다 — 서버가 `/shared/element-match.js`로 그대로 내려준다
>   (`server/index.js`의 특례 1줄). 복사해 두면 서식 복사와 일괄 수정이 서로 다른 요소를
>   건드리는, 추적이 매우 어려운 버그가 된다.
> - **도구 4개**(`core/tools/slide.tools.js`):
>   `list_element_keys(service_id|slide_ids)` → 키·라벨·장수·표본 (CLI/MCP가 뭘 고칠지 알아내는 입구)
>   `update_elements(slide_ids|service_id, key, patch)` → **patch에 준 필드만** 덮어쓴다(값 null=필드 삭제)
>   `remove_elements(...)` · `add_element_to_slides(..., element, skip_existing?)`
>   짝 없는 슬라이드는 안 건드리고 `changed/matched/skipped`를 항상 돌려준다.
> - **편집기 UI**: 요소를 고르면 우측 패널 맨 위에 **🔗 `<요소> · N장에 있음`**.
>   이동·크기·색·글꼴·줄간격·투명도 변경이 그 장들에 함께 간다
>   (`commitEls(fields)`가 바뀐 필드 이름을 받아 `update_elements`로 전파 — 요소를 통째로
>   복사하지 않는다). 범위는 `이 예배 전체 / 선택한 장만`.
> - **여러 장을 골라 뒀으면 기본으로 켜진다**(`batchOn()`). 처음엔 체크박스를 매번 켜야 했는데,
>   **배경(`set_slide_background`)은 이미 멀티셀렉만으로 전체 적용**되므로 요소만 토글을
>   요구하는 건 일관성이 없다 — 사용자는 "골라 놨으니 당연히 이 장들에 적용되겠지"로 읽는다
>   (실제로 그렇게 기대해서 "안 된다"는 보고가 왔다). 한 장만 선택했을 때는 **꺼짐** —
>   그 상태의 범위는 "예배 전체"라 켜져 있으면 한 장을 만졌는데 44장이 조용히 바뀐다.
>   사용자가 직접 끈 값은 `onFor`(선택 서명)로 그 선택 동안만 유지한다.
> - **안전장치 3가지 (전부 실측으로 걸러낸 것)**:
>   ① **글자(text)는 자동 전파에서 뺀다** — 가사 상자를 옮기려고 켜 둔 채 가사를 고치면
>      12장이 전부 같은 가사가 된다. 글자는 `글자도 똑같이` 버튼 + 확인창으로만.
>   ② **범위 기본값은 선택을 따른다**(`batchScope()`): 여러 장을 골라 뒀으면 `선택한 장만`,
>      아니면 `이 예배 전체`. 여러 장 고르는 행위 자체가 "이 장들에 하겠다"는 뜻이다.
>      드롭다운으로 직접 고른 값은 **그 선택 상태에서만** 유지한다(`scopeFor` = 선택 서명) —
>      선택이 바뀌면 기본으로 되돌아간다. 안 그러면 예전 선택에 맞춘 범위가 눌러앉아 엉뚱한 장을 건드린다.
>      범위 자체는 **엄격히** 지킨다 — "선택한 장만"인데 선택이 1장이면 대상 0장(아무 일도 안 일어남).
>      예전 초안은 이때 조용히 "예배 전체"로 되돌아가 **의도와 정반대로 번졌다**.
>      패널 표시 여부는 **예배 전체 기준**으로 판단한다(범위 기준이면 패널이 사라져 이유를 알 수 없다).
>   ③ `add_element_to_slides`의 "이미 있으면 건너뛰기"는 `matchKeys`로 판정하면 **안 된다** —
>      이미지가 있는 장에 이미지를 더하면 `type:image:2`가 되어 기존 `type:image:1`과 안 겹쳐
>      **항상 통과**한다(두 번 돌리면 로고가 겹친다). bind 이름 → 종류+url → 종류+글자로 본다.
>      실측: 1회차 84장 추가·2장 건너뜀 → 2회차 **0장**(멱등).
> - 확인창 숫자는 **키가 실제로 있는 장수**여야 한다(대상 id 수를 쓰면 "85장을 바꿉니다"라 하고
>   실제로는 24장만 바뀐다 — 실측으로 걸림).

> ⚠️ **v4.18 (휴대폰 "순서" 탭에서 두 번 탭이 안 먹던 문제)** — 구현 기준(현행)
> - 증상: 휴대폰 순서 탭에서 슬라이드를 두 번 탭해도 발표로 안 넘어갔다(데스크톱 더블클릭은 정상).
> - 원인: `wireDoubleTap`이 `lastAt`을 **노드 클로저**에 뒀다. 그런데 첫 탭 → `onRowClick` →
>   **`render()`가 목록을 통째로 다시 그린다** → 두 번째 탭은 방금 만들어진 **새 요소**에 떨어지고
>   그 요소의 `lastAt`은 0이다. 즉 **영원히 "첫 탭"** 이었다.
>   (실측: 첫 탭 후 `before === after` false · `before.isConnected` false)
>   데스크톱이 멀쩡했던 건 `dblclick`이 브라우저 기본 이벤트라 **요소 정체성이 아니라 위치·시간**으로
>   판정하기 때문. 그래서 "데스크톱에서 되니까 됐겠지"가 통하지 않는다.
> - 수정: 상태를 **모듈 스코프 `lastTap = {key, at}`** 으로 빼고 `key = 슬라이드 id`를 쓴다
>   → 다시 그려도 같은 슬라이드면 같은 키. 리스트·타일 **양쪽 다** 같은 구조였으므로 함께 고쳤다.
> - 검증(탭 이벤트 합성): 같은 행 빠른 두 탭 → 발표 ✓ / 600ms 간격 → 무시 ✓ /
>   다른 행 한 번씩 → 무시 ✓ / 버튼(◉) 뒤 행 탭 → 무시 ✓.

> ⚠️ **v4.17 (아이패드: 메뉴가 안 닫힘 · 순서 목록 글자 잘림)** — 구현 기준(현행)
> - **메뉴에 갇히는 문제**: 바깥을 눌러 메뉴를 닫는 건 `document`의 **click**에 걸려 있었다.
>   **iOS 사파리는 버튼·링크가 아닌 요소를 탭했을 때 document까지 click을 올려보내지 않는다** →
>   아이패드에선 캔버스를 눌러도 ⚙예배 메뉴가 안 닫히고, 키보드가 없어 Esc도 못 쓴다(= 갇힘).
>   → `pointerdown`으로도 같은 핸들러를 건다(`closeMenusOnOutside`). 메뉴 버튼 자신은
>   `panel.parentElement`(.menu) 안이라 여기 안 걸려 **토글이 그대로** 동작한다.
>   모달 8개의 배경 클릭 닫기도 `mousedown` → `pointerdown`으로 통일.
>   **속성 서랍**(≤1200px)은 배경막이 없어 바깥을 눌러도 안 닫혔다 → 같은 방식으로 닫는다.
>   단 휴대폰 탭 모드(`body[data-pane]`)에서는 제외 — 탭 상태와 어긋난다.
> - **순서 목록이 잘리던 문제**(≤1200px): 제목이 **29px**만 받아 전부 "…"였다(실측 필요 160~184px).
>   ① 칼럼 240→**272px**, 썸네일 84→**52px**(한 줄의 36%를 먹던 진짜 원인), num·gap 축소.
>   ② **버튼 4개를 2×2로 접는다** — 아이패드엔 `body.touch`로 ▲▼가 추가돼 `▲ ▼ ◉ ✕`가
>      한 줄에 늘어서고, 이때 제목은 **21px**까지 줄어든다(데스크톱엔 ▲▼가 없어 이 상태가 안 보인다).
>      `.row-acts` 래퍼 + `grid-auto-flow: column` / `grid-template-rows: repeat(2,auto)`,
>      `.movers`는 `display: contents`로 껍데기를 없애 ▲▼가 그대로 칸에 들어간다. 배치는
>      `▲ ◉ / ▼ ✕` — ▲ 위·▼ 아래라 방향 은유가 맞아 잘못 눌러 순서가 틀어질 여지가 적다.
>   ③ 제목 **두 줄까지**(`-webkit-line-clamp: 2`).
>   결과 제목 21→**98px**, 행 높이 51~69px. **52px가 실측 분기점**(60px→2개 잘림 / 56→1 / 52→0 /
>   48은 더 줄여도 이득 없음). 78장 중 4장만 남는데 전부 26~30자 광고 문장이라 두 줄로도 식별은 된다.
> - **`.del`(✕) 표시 판정은 `body.touch` 하나로 통일**: 터치엔 hover가 없어 ✕가 영영 안 보이면서
>   자리만 차지했다. `@media (hover:none)`만 쓰면 2×2 배치(`body.touch`)와 신호가 달라져
>   **터치 가능한 노트북(마우스 사용)에서 "칸은 잡혔는데 ✕만 안 보이는"** 상태가 된다(실제로 재현됨).

> ⚠️ **v4.16 (리뷰용 PDF 내보내기)** — 구현 기준(현행)
> - 왜: 예배 **준비 중**인 자료를 2~3명에게 보여주고 의견을 받아야 한다. 리뷰어는 각자 집에서
>   **휴대폰**으로 본다. 실시간 공유(터널·Cloudflare Pages·코멘트 DB)까지 설계했으나
>   **PDF 한 파일만** 만드는 쪽으로 결정 — 카톡에 올리면 미리보기가 뜨고 탭하면 바로 넘겨본다.
>   추가 인프라 0, 맥북을 켜둘 필요 0, 앱 설치 0. 잃는 건 실시간성뿐이고 준비 단계엔 스냅샷이면 충분하다.
> - **크롬 `--print-to-pdf`가 이미 이미지 내보내기의 중간 산출물**이었다 → 쪼개지 않고 그대로 내보낸다.
>   그래서 이미지 경로와 달리 **poppler가 필요 없다**(크롬만). `renderPdf()`를 뽑아 두 경로가 공유.
> - `export_review_pdf(service_id, slide_ids?, include_hidden?, numbers?)` → `data/exports/<이름>.pdf`.
>   HTTP `POST /api/export/pdf`가 `Bun.file()`로 **스트리밍**(수십 MB를 브라우저 메모리에 안 들게 —
>   패키지 내보내기와 같은 이유). 편집기 ⚙예배▾ "📄 PDF로 내보내기", 여러 장 선택 시 그 장만.
> - **장 번호(`numbers`, 기본 켬)**: 코멘트 기능이 없으므로 리뷰어가 "12번 오타"처럼 지목할 **유일한 수단**.
>   `/export/?numbers=1`일 때만 `.page-no`(오른쪽 아래 반투명 알약)가 붙는다 — 이미지 내보내기엔 안 붙는다.
>   인쇄 시 알약 배경이 빠지지 않게 `print-color-adjust: exact`(빠지면 밝은 배경에서 번호가 안 보인다).
> - 배경 영상은 기존 `withPosterBackgrounds`가 대표 프레임으로 바꿔주므로 **그대로 해결**된다(검게 안 나옴).
> - **숨긴 장(⊘) 처리** — 이미지 내보내기도 함께 바뀜:
>   ① ⚙예배▾ 에 **"숨긴 장도 내보내기"** 체크박스(`export-hidden`) → 전장 내보낼 때 `include_hidden`으로 전달.
>   ② **장을 직접 골랐으면 고른 그대로 나간다**(숨김이라도). 예전엔 hidden 필터가 ids 필터보다 **먼저** 걸려서
>      숨긴 장을 ⌘클릭해 내보내면 **조용히 빠졌다**(2장 고르면 0장). `pickSlides()`와 `client/export/export.js`가
>      **같은 순서**여야 한다 — 다르면 쪽수가 어긋나 `warning`이 뜬다.
>   실측(Base 79장 중 숨김 9): 기본 70 · `include_hidden` 79 · 숨긴 장 2개 직접 선택 2(예전 0).
> - 실측: 118장 → **11.5MB · 25초**. 카톡 파일 전송(300MB)·메일 첨부 모두 여유.
> - **풀다운 정리(같이 수행)** — 내보내기가 3종이 되면서 ⚙예배 메뉴가 23행/726px로 불어났다.
>   ① **📤 내보내기 모달**(`#export-modal`)로 3종 통합 → 메뉴는 `📤 내보내기…` 1줄.
>      모달 = **대상**(전체 N장 / 선택한 N장 / ☐숨긴 장 포함, 없으면 자동 숨김·비활성) + **형식 카드 3장**.
>      `exportTarget()`이 라디오를 읽어 세 함수(`exportPdf`/`exportImages`/`exportService`)가 공유한다.
>      예전엔 "여러 장 골라두면 자동으로 그 장만"이라 **눌러보기 전엔 무엇이 나올지 몰랐다** → 명시적 선택으로.
>   ② 메뉴 섹션에 라벨을 전부 달았다: ⚙예배 = `모양`/`이 예배`/`파일`/`관리`,
>      ＋추가 = `기본 종류`/`내 디자인`/`찾아서 추가`(`renderAddMenu`가 kind 바뀔 때 라벨 삽입).
>      `.menu-label:not(:first-child)`가 **구분선을 겸한다** — 라벨과 `menu-sep`을 둘 다 쓰면
>      여백만 벌어지고 경계는 오히려 흐려진다.

> ⚠️ **v4.15 (배경 영상 시작 깜박임 제거)** — 구현 기준(현행)
> - 증상: 배경 영상이 처음 나오는 슬라이드에서 영상이 늦게 시작해 검은 순간이 스쳤다.
> - 원인: `makeStageDecoded`가 **이미지만** 기다렸다(`img.decode`). 영상은 새 스테이지를 화면에
>   붙인 **뒤에야** 로드를 시작해서 그 사이가 비었다.
> - 해결 두 겹:
>   ① **첫 프레임을 미리 기다린다** — `waitVideoReady(root, 400)`를 화면 교체 전에 부른다.
>      실측: 영상 요소가 붙는 시점에 이미 readyState≥2 → 깜박임이 아예 안 생긴다(107ms).
>      400ms를 넘기면 그냥 진행한다(예배 중엔 늦게 넘어가는 게 깜박임보다 나쁘다).
>   ② **페이드인(안전망)** — `fadeInVideo()`가 `.bg-fade`(opacity 0)로 시작해 `loadeddata`에
>      해제, CSS `transition: opacity 260ms`. 큰 파일이라 ①로 못 덮을 때를 위한 것.
>      깨진/아주 느린 파일은 1.2초 뒤 강제 표시(배경이 영영 검게 남지 않게).
>   - CSS 기본값은 `opacity: 1`이고 **JS가 `.bg-fade`를 붙였을 때만** 감춘다 — 핸드셰이크를
>     안 타는 경로(내보내기 폴백 등)에서 영상이 안 보이는 사고를 막기 위해.
>   - 이미지 내보내기는 굽기 전에 `.bg-fade`를 강제 해제한다(ffmpeg 없어 `<video>`로 폴백될 때 대비).
> - 같은 영상이 이어지는 슬라이드는 배경 레이어를 hoist해 살려두므로 **페이드가 다시 일어나지 않는다**
>   (v4.10). 영상이 처음 등장할 때만 부드럽게 시작한다.

> ⚠️ **v4.14 (성경 자동 분할 용량 보정)** — 구현 기준(현행)
> - 증상: 한 장에 충분히 들어갈 본문이 두 장으로 나뉘었다.
> - 원인: `bibleAutoCapacity`가 **한글 1글자 = 1em**으로 잡았다. 한글 문장은 어절마다 공백이 있고
>   공백은 훨씬 좁아 **평균 0.76em**이다. 게다가 절마다 이미 `+3`(절 번호·공백 몫)을 빼면서
>   별도 안전계수 0.92까지 곱해 **이중 차감**했다. → 실제 용량의 68%만 사용.
> - 보정 근거는 **추정이 아니라 실측**이다. `layer-renderer`로 실제 그려 넘치는 지점을 이분탐색:
>   한 장에 1절 123자 · 2절 120 · 3절 120 · 4절 112 (절이 늘수록 절 번호·줄 끝 여백으로 줄어든다).
>   → `KO_ADVANCE = 0.78`로 한 줄 글자 수를 구하고, 안전계수는 **없앤다**
>   (`perLine·lines`의 floor와 `+3/절`이 이미 여유를 만든다: 1절 117 / 2절 114 / 3절 111 / 4절 108 ≤ 실측).
> - `BIBLE_AUTO_MAX_VERSES` 4 → 6 (짧은 절이 5~6개일 때 길이는 남는데 절 수로 나뉘던 것 방지).
> - 효과(성경 400개 장 검증): 슬라이드 **5,930 → 4,222장(-28.8%)**, 99%의 장에서 감소.
>   예: 빌립보서 4:6-7(108자) 2장 → 1장.
> - 여전히 나뉘는 경우는 실제로 안 들어가는 것이다(절 하나가 60~70자면 두 절 합쳐 128자 > 120).
>   더 담으려면 템플릿의 글자 크기를 줄이거나 본문 박스를 키우면 용량이 자동 재계산된다.
