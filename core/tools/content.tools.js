...
// and inserts the resulting slides into a scene.
...
function requireScene(db, sceneId) {
  if (!db.query("SELECT id FROM scenes WHERE id = ?").get(sceneId)) {
...
register({
  ...
  description: "지정한 성경 본문(책/장/절 범위)을 씬에 본문 슬라이드로 추가한다. 절 수에 따라 자동 분할된다.",
  input_schema: {
    type: "object",
    properties: {
  ...
  handler: ({ scene_id, book, chapter, verse_start, verse_end, layout }, { db }) => {
    requireScene(db, scene_id);
    ...
    return { slide_ids: insertSlides(db, scene_id, slides) };
  },
});

register({
  name: "add_hymn_slides",
  description: "찬송가 번호를 받아 가사 슬라이드를 씬에 추가한다. 절 선택과 슬라이드당 줄 수를 지정할 수 있다.",
  input_schema: {
    type: "object",
    properties: {
      scene_id: { type: "string" },
    ...
    required: ["scene_id", "number"],
  },
  handler: ({ scene_id, number, verse_nos, lines_per_slide }, { db }) => {
    requireScene(db, scene_id);
...
register({
  ...
  description: "교독문 번호를 받아 교독문 슬라이드를 씬에 추가한다(인도자/회중 교대로 분할).",
  input_schema: {
    type: "object",
    properties: {
      scene_id: { type: "string" },
  ...
  handler: ({ scene_id, number, segments_per_slide }, { db }) => {
    requireScene(db, scene_id);
...
register({
  ...
  description:
    "찬양팀 찬양 가사를 구조화된 sections로 받아 슬라이드를 씬에 추가한다. " +
  ...
  input_schema: {
    ...
    required: ["scene_id", "title", "sections"],
  },
  handler: ({ scene_id, title, sections, lines_per_slide }, { db }) => {
    requireScene(db, scene_id);
...
register({
  ...
  description: "광고 항목 배열을 받아 광고 슬라이드 하나를 씬에 추가한다.",
  input_schema: {
    type: "object",
    properties: {
      scene_id: { type: "string" },
    ...
    required: ["scene_id", "items"],
  },
  handler: ({ scene_id, items, title }, { db }) => {
    requireScene(db, scene_id);
    return { slide_ids: insertSlides(db, scene_id, [{ template_type: "announcement", data: { title, items } }]) };
  },
});
